	...
	void CreateGrid(){
		...
		for(int x = 0; x < Size.x; x++){
			for(int z = 0; z < Size.z; z++){
				...
				// worry about.
				newCell = (Transform)Instantiate(CellPrefab, new Vector3(x * walheight*4, 0, z*walheight*4), Quaternion.identity);
				...
				// We assign it here.
				newCell.GetComponent<Cell_Script>().Position = new Vector3(x, 10, z);
...
//	Camera.mainCamera.orthographicSize= Mathf.Max(Size.x, Size.z);
	...
	void SetRandomNumbers(){
		...
		foreach(Transform child in transform){
			...
			//..and Weight variable in the Cell_Script.
			child.GetComponent<Cell_Script>().Weight = weight;
		}
	}
	...
	void SetAdjacents(){
		...
		for(int x = 0; x < Size.x; x++){
			for(int z = 0; z < Size.z; z++){
				...
				Cell_Script cScript = cell.GetComponent<Cell_Script>();
	...
	int SortByLowestWeight(Transform inputA, Transform inputB){
		...
		int a = inputA.GetComponent<Cell_Script>().Weight; //a's weight
		int b = inputB.GetComponent<Cell_Script>().Weight; //b's weight
	...
	void AddToSet(Transform toAdd){
		...
		foreach(Transform adj in toAdd.GetComponent<Cell_Script>().Adjacents){
			//Add one to the adjacent's cellScript's AdjacentsOpened
			adj.GetComponent<Cell_Script>().AdjacentsOpened++;
			//If
			...
			// then..
			if(!Set.Contains(adj) && !(AdjSet[adj.GetComponent<Cell_Script>().Weight].Contains(adj))){
				//.. Add this new cell into the proper AdjSet sub-list.
				AdjSet[adj.GetComponent<Cell_Script>().Weight].Add(adj);
			}
		}
	}
	...
	void FindNext(){
		...
		do{
			...
			//	Debug.Log("We're Done, "+Time.timeSinceLevelLoad+" seconds taken"); 
				...
				Instantiate (Portal,new Vector3 ( Set[Set.Count-1].transform.position.x, 
				                                 Set[Set.Count-1].transform.position.y+walheight*3,
			...
			//	Debug.Log("here is " + here); 
				...
				foreach(Transform cell in Grid){
					if(!Set.Contains(cell)){
						cell.Translate(Vector3.up*walheight*4);	
		...
		}while(next.GetComponent<Cell_Script>().AdjacentsOpened >= 2);
